...
export const getWeakSubjectMaterial = async (req, res) => {
  ...
  try {
    ...
    if (!collegeInfo) {
      return res
        .status(404)
        .json({ message: `No college info found for user ID ${userId}` });
    }
    ...
    if (!userSubjects) {
      return res
        .status(404)
        .json({ message: "No selected subjects found for this user." });
    }

    console.log("userSubjects:", JSON.stringify(userSubjects));

    const selectedSubjectIds = userSubjects.selectedSubjects.map(
    ...
    const selectedSubjectCodes = selectedSubjects.map(
    ...
    console.log(
      "subjectDependencies:",
      JSON.stringify(subjectDependencies, null, 2)
    );

    // Filter dependencies that match weak subjects
    const dependentSubjects = subjectDependencies.flatMap((dep) =>
      dep.dependencies.filter((dep) =>
        weakSubjectCodes.includes(dep.subjectCode)
      )
    );

    console.log(
      "dependentSubjects:",
      JSON.stringify(dependentSubjects, null, 2)
    );

    const materialSubjectCodes = [
      ...new Set([
        ...weakSubjectCodes,
    ...
    // Fetch current semester subject materials
    const currentSemesterMaterials = await SubjectMaterial.findOne({
      semester: currSemester.toString(),
    }).exec();

    console.log(
      "currentSemesterMaterials:",
      JSON.stringify(currentSemesterMaterials, null, 2)
    );

    // Prepare the response
    const response = {
      weakMaterials: subjectMaterials.flatMap((material) =>
        material.subjects
          .filter((subject) =>
            materialSubjectCodes.includes(subject.subjectCode)
          )
          .map((subject) => ({
      ...
      currentSemesterMaterials: currentSemesterMaterials
        ? currentSemesterMaterials.subjects.map((subject) => ({
            subjectCode: subject.subjectCode,
            subjectName: subject.subjectName,
            materialUrl: subject.subjectUrl,
            courses: subject.courses,
    ...
    res.status(200).json({
      message:
        "Weak subject materials and current semester materials retrieved successfully",
      materials: response,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({
        message: "An error occurred while retrieving weak subject materials",
      });
  }
};
