...
import { IUser, ResetPasswordProps, profileProps } from "../../../@types/auth";
import {
  setUserToken,
  setIsLoggedIn,
  setUserInfo,
...
// Convert async storage retrieval to use localStorage
export const getUserTokenFromState = () => async (dispatch: AppDispatch) => {
  const user_token = getItemFromStorage("user_token");
  console.log("this is token ", user_token);

  dispatch(setUserToken(user_token));
};

export const getIsLoggedIn = () => async (dispatch: AppDispatch) => {
  const is_logged = getItemFromStorage("is_logged");
  dispatch(setIsLoggedIn(is_logged));
};

export const getUserData = () => async (dispatch: AppDispatch) => {
  const user_auth_data = getItemFromStorage("set_user_auth_data");
...
export const userAUTHORIZED =
  ...
  async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    const validateUser = is_logged ? JSON.parse(is_logged) : null;
    if (validateUser) {
