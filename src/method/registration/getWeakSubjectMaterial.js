...
export const getWeakSubjectMaterial = async (req, res) => {
    ...
    try {
        ...
        const currentSemesterMaterials = await SubjectMaterial.findOne({ semester: currSemester.toString() }).exec();

        console.log('currentSemesterMaterials:', JSON.stringify(currentSemesterMaterials, null, 2));
        ...
        const response = {
            ...
            currentSemesterMaterials: currentSemesterMaterials ? currentSemesterMaterials.subjects.map(subject => ({
                subjectCode: subject.subjectCode,
                subjectName: subject.subjectName,
                materialUrl: subject.subjectUrl,
                courses: subject.courses
        ...
        res.status(200).json({
            message: "Weak subject materials and current semester materials retrieved successfully",
            materials: response
        });
